        ...
        if data:
            for name, info in data.items():
                ...
                tr.update({
                    ...
                    "volume": info.get("volume", 100)      # <-- AÑADIR
                })
...
def send_pattern_state_to_pd(pattern_index):
    ...
    for track_idx, params, lfo_mode, volume in snapshot:
        for param_name, value in params.items():
            safe_send_osc(
                osc_pd_client,
                f"/{param_name}",
                [track_idx, value / 100.0]
            )

        safe_send_osc(osc_pd_client, "/lfo_mode", [track_idx, lfo_mode])
        safe_send_osc(osc_pd_client, "/volumen", [track_idx, volume / 100.0])

    safe_send_osc(osc_pd_client, "/master_vol", master / 100.0)
...
def update_master_volume(val):
    ...
    safe_send_osc(osc_pd_client, "/master_vol", value / 100.0)
    send_display(f"Master: {value}")
...
def update_track_volume(val):
    with state_lock:
        ...
        value = int(val)
        tracks[track_name]["volume"] = value
        track_idx = track_mapping[track_name]

    safe_send_osc(osc_pd_client, "/volumen", [track_idx, value / 100.0])
    send_display(f"Vol {track_idx}: {value}")
...
class RhythmUI(BoxLayout):
    def __init__(self, **kwargs):
        ...
        # MASTER
        master_box = BoxLayout(orientation="vertical", size_hint_x=0.25)
        master_box.add_widget(Label(text="Master", size_hint_y=None, height=18))
        ...
        self.master_slider.bind(
            value=lambda s, v: update_master_volume(v)
        )

        master_box.add_widget(self.master_slider)
        global_row.add_widget(master_box)
        ...
        # ---------- VOLUMEN DE LA PISTA ----------
        vol_row = BoxLayout(size_hint_y=None, height=40, spacing=6)

        vol_row.add_widget(Label(text="Volume", size_hint_x=0.2))

        self.track_volume_slider = Slider(
            min=0,
            max=100,
            value=100,
            step=1
        ...
        self.track_volume_slider.bind(
            value=lambda s, v: update_track_volume(v)
        )

        vol_row.add_widget(self.track_volume_slider)

        editor.add_widget(vol_row)
    ...
    def set_param_sliders(self, track_name):
        ...
        try:
            with state_lock:
                ...
                volume = tracks[track_name].get("volume", 100)
